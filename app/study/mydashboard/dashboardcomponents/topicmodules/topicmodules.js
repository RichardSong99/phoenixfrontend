...
const TopicModules = () => {
  ...
  useEffect(() => {
    if (Array.isArray(topicSummaryList)) {
      if(topic !== "All") {
        setDisplayTopicSummaryList(filterTopicSummaryListByCategory(topic));
      } else {
        setDisplayTopicSummaryList(topicSummaryList);
      }
    }
  }
  , [topicSummaryList, topic]);
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4">
      <div className="w-[95%] flex flex-row justify-around items-center font-semibold rounded-[10px]">
          <div
              className={`cursor-pointer text-[13px] h-[30px] w-[20%] flex justify-center items-center ${topic === 'All' ? 'border-b-[2px] border-themeLightGreen text-themeLightGreen' : 'text-themeDarkGray'}`}
              onClick={() => setTopic("All")}
          >
              All
          </div>
          <div
              className={`cursor-pointer text-[13px] h-[30px] w-[20%] flex justify-center items-center ${topic === 'Algebra' ? 'border-b-[2px] border-themeLightGreen text-themeLightGreen' : 'text-themeDarkGray'}`}
              onClick={() => setTopic("Algebra")}
          >
              Algebra
          </div>
          <div
              className={`cursor-pointer text-[13px] h-[30px] w-[20%] flex justify-center items-center ${topic === 'Advanced math' ? 'border-b-[2px] border-themeLightGreen text-themeLightGreen' : 'text-themeDarkGray'}`}
              onClick={() => setTopic("Advanced math")}
          >
              Advanced Math
          </div>
          <div
