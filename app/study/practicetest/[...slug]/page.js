...
export default function Page() {
    ...
    const loadTest = async () => {
        ...
        try {
            ...
            const foundTest = testUnderlyingList.find(item => item.Test.Name === testName);
            if (foundTest) {
                setTestObj(foundTest);
            } else {
                const response = await fetchTestByName({ testName: testName });
                setTestObj(response);
            }
        }catch (error) {
            console.log(error);
    ...
    return (
        <>
            {test && !test?.Completed && <ActiveTestCommander test={testObj.Test} />}
        </>
    );
}